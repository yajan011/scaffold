...
each ``gen_ai.*`` span into a :class:`~ankora.models.Case`.
...
# Re-exported so ``from ankora.ingest.otel import IngestResult`` keeps working.
__all__ = ["IngestResult", "ingest_otel", "is_otel_shape"]

GEN_AI_PREFIX = "gen_ai."
_REQUEST_PREFIX = "gen_ai.request."


def is_otel_shape(data: Any) -> bool:
    """True if parsed JSON looks like an OTel span export (not Langfuse).

    OTel spans carry ``attributes`` / ``spanId``; Langfuse observations never
    do, which keeps the two formats cleanly distinguishable.
    """
    if isinstance(data, dict):
        ...
        if isinstance(data.get("spans"), list):
            return True
        return _strict_span_like(data)
    if isinstance(data, list):
        return any(_strict_span_like(item) for item in data)
    return False


def _strict_span_like(obj: Any) -> bool:
    # Only the OTel-exclusive markers (attributes / spanId). NOT traceId, which
    # Langfuse observations also carry.
    return isinstance(obj, dict) and any(k in obj for k in ("attributes", "spanId", "span_id"))


def ingest_otel(trace_file: str | Path) -> IngestResult:
    """Parse an OTel GenAI trace export into Cases.

    Returns an :class:`~ankora.ingest.IngestResult` with the produced Cases, the
    number of spans seen (``total``), and how many were skipped (non-``gen_ai``
    or lacking any usable input/output). Raises ``FileNotFoundError`` if the file
    is missing and ``json.JSONDecodeError`` if it is not valid JSON — callers
    (the CLI) render those into friendly messages.
    """
    ...
    return IngestResult(cases=cases, total=len(spans), skipped=skipped)
