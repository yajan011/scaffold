...
# The default config written by `evalgate init`; mirrors the CLAUDE.md example.
DEFAULT_CONFIG_YAML = """\
version: 1
suites: ["evals/**/*.yaml"]
target:
...
def _humanize_validation_error(error: ValidationError, source: str) -> ConfigError:
    ...
    lines = [f"Invalid evalgate config ({source}):"]
...
def load_config(path: str | Path = "evalgate.yaml") -> Config:
    """Read an evalgate.yaml file and validate it into a :class:`Config`.
    ...
    try:
        ...
        raise ConfigError(
            f"Config file not found: {config_path}. Run `evalgate init` to create one."
        ) from exc
